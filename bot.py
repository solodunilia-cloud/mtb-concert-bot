...
# -*- coding: utf-8 -*-
"""
MTB Concerts Bot — финальная версия
"""
    ...
    with sqlite3.connect(DB_PATH) as conn:
        ...
        if data.get('id'):
            conn.execute('''UPDATE concerts SET
                artist=?, date=?, time=?, poster_status=?,
...
def db_all(include_cancelled=False) -> List[dict]:
    with sqlite3.connect(DB_PATH) as conn:
        ...
        if include_cancelled:
            rows = conn.execute('SELECT * FROM concerts ORDER BY date ASC, id DESC').fetchall()
        else:
            rows = conn.execute(
...
def extract_date_time(text: str) -> Tuple[Optional[str], Optional[str]]:
    ...
    if not date_str:
        m = re.search(r'(\d{1,2})\s+(' + '|'.join(MONTHS) + r')(?:\s+(\d{4}))?', text.lower())
        if m:
...
def detect_kw(text: str) -> Optional[Tuple[str, str]]:
    """Находит действие через точное совпадение или fuzzy."""
    text_n = norm(text)
    words  = text_n.split()
...
def parse_trigger(text: str) -> Optional[dict]:
    ...
    # Убираем из текста всё лишнее — остаток это артист
    ...
    if action == 'text':
        # Всё после ключевого слова = payload
        text_n  = norm(text)
        kw_n    = norm(found_kw)
        ...
        payload = text  # проверим POSTER_OK внутри apply_action
...
async def apply_action(upd: Update, ctx: ContextTypes.DEFAULT_TYPE,
    ...
    if action == 'cancel':
        ...
        if not url:
            await msg.reply_text(f"Нет ссылки. Пример:\n`{name} билеты https://...`",
                                 parse_mode='Markdown')
            return
        ...
        if len(payload) < 10:
            await msg.reply_text(f"Текст слишком короткий. Пример:\n`{name} текст Описание...`",
                                 parse_mode='Markdown')
            return
        ...
        if not d:
            await msg.reply_text(f"Не распознал дату. Пример:\n`{name} дата 15.04.2026 21:00`",
                                 parse_mode='Markdown')
            return
...
async def cmd_start(upd: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await upd.message.reply_text(
        ...
        "`/edit` — редактировать / добавить данные\n"
...
async def cmd_new(upd: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not args:
        ...
        await upd.message.reply_text("Введи имя артиста:")
        return

    d, t    = extract_date_time(args)
    cleaned = re.sub(r'\d{1,2}[./\-]\d{1,2}[./\-]\d{4}', '', args)
    cleaned = re.sub(r'\b\d{1,2}[:.]\d{2}\b', '', cleaned).strip()
    artist  = cleaned or args
...
async def cmd_edit(upd: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await upd.message.reply_text(
        "Выбери мероприятие чтобы редактировать или добавить афишу / билеты / текст / дату:",
        reply_markup=InlineKeyboardMarkup(kb)
    )

async def cmd_list(upd: Update, ctx: ContextTypes.DEFAULT_TYPE):
    register_chat(upd.effective_chat.id)
    inc_all      = bool(ctx.args and ctx.args[0].lower() == 'all')
    month_filter = ctx.args[0] if ctx.args and re.match(r'^\d{4}-\d{2}$', ctx.args[0]) else None
...
async def cmd_code(upd: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    artist = c.get('artist', '')
    dt     = f"{c.get('date', '')} {c.get('time') or ''}".strip()
    url    = c.get('tickets_url', '')
    desc   = c.get('description_text', '')
  ...
  <div class="event-content">
    ...
    <div class="buttons-row">
      <button class="buy-btn" onclick="window.open('{url}','_blank')">Купить билет</button>
    </div>
    <div class="event-description"><p>{desc}</p></div>
  </div>
</div>"""
