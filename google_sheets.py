...
class GoogleSheetsManager:
    ...
    def _get_or_create_archive_sheet(self):
        try:
            return self.spreadsheet.worksheet('Архив')
        except Exception:
            ws = self.spreadsheet.add_worksheet('Архив', rows=1000, cols=COL_COUNT)
            ws.update('A1:K1', [['Сайт', 'Дата', 'Время', 'Страничка', 'Артист',
                                  'Покупка билета', 'Картинка', 'Текст', 'Афиша', 'Статус', 'ID']])
            ws.format('A1:K1', {
                'backgroundColor': C_HEADER,
    ...
    def _sync_data_row(self, concert: Dict):
        ...
        # published/cancelled → Архив, активные → Данные
        if status_val in ('published', 'cancelled'):
            self._move_to_archive(concert)
            return
        ...
        # Чередование по порядковому номеру данных (строка 2 = первая запись = чётная → тёмная)
        data_row_num = row_idx - 1  # 1 = первая запись данных
        bg = C_BLACK if data_row_num % 2 == 1 else C_DARKGRAY
    ...
    def _move_to_archive(self, concert: Dict):
        """Переносит концерт из листа Данные в Архив."""
        try:
            status_val = concert.get('status', 'draft')
            cid        = str(concert.get('id', ''))
            slug       = concert.get('slug', '') or ''
            ...
            # Удаляем из Данные если была там
            data_ws  = self._get_or_create_data_sheet()
            ...
            # Добавляем в Архив
            arch_ws    = self._get_or_create_archive_sheet()
            arch_vals  = arch_ws.get_all_values()
            arch_row   = None
            for i, row in enumerate(arch_vals[1:], start=2):
                if len(row) >= 11 and row[10] == cid:
                    arch_row = i
                    break
            if arch_row:
                arch_ws.update(f'A{arch_row}:K{arch_row}', row_data)
                new_idx = arch_row
            else:
                arch_ws.append_row(row_data[0])
                new_idx = len(arch_ws.get_all_values())

            bg = C_BLACK if (new_idx - 1) % 2 == 1 else C_DARKGRAY
            arch_ws.format(f'A{new_idx}:K{new_idx}', {
                'backgroundColor': bg,
                'textFormat': {'foregroundColor': C_LIGHT, 'fontSize': 9},
                'verticalAlignment': 'MIDDLE',
            })
        except Exception as e:
            logger.error(f"_move_to_archive error: {e}")
    ...
    def _draw_calendar(self, ws, month: int, year: int, all_concerts: List[Dict]):
        ...
        # Сначала красим ВСЕ ячейки календаря тёмным фоном
        total_rows = len(cal) * 4
        requests.append({'repeatCell': {
            'range': {'sheetId': sheet_id, 'startRowIndex': 2, 'endRowIndex': 2 + total_rows,
...
def parse_trigger(text: str) -> Optional[dict]:
    ...
    # Для text и tickets — пересчитываем artist из части ДО ключевого слова
    # делаем это ДО проверки длины, т.к. cleaned может быть длинным
    if action == 'text':
        for kw in KW['text']:
            ...
            if m:
                ...
                # убираем кавычки из начала payload
